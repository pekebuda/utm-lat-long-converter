...
'''
    ...
    def natoToUtm(self, utme, utmn, utmz, latz, digraph):
        ...
        var eidx = self.digraphLettersE.indexOf(eltr);
        var nidx = self.digraphLettersN.indexOf(nltr);
        ...
        var latBand = self.digraphLettersE.indexOf(latz);
    ...
    def utmToNato(self, x, y, utmz, southern):
        ...
        '''
        var esq = (1 - (self.b / self.a) * (self.b / self.a));
        var e0sq = self.e * self.e / (1 - Math.pow(self.e, 2));
        var e1 = (1 - Math.sqrt(1 - Math.pow(self.e, 2))) / (1 + Math.sqrt(1 - Math.pow(self.e, 2)));
        var M0 = 0;
        ...
        #
        var mu = M / (self.a * (1 - esq * (1 / 4 + esq * (3 / 64 + 5 * esq / 256))));
        ...
        var N1 = self.a / Math.sqrt(1 - Math.pow(self.e * Math.sin(phi1), 2));
        var R1 = N1 * (1 - Math.pow(self.e, 2)) / (1 - Math.pow(self.e * Math.sin(phi1), 2));
        var D = (x - 500000) / (N1 * self.k0);
        var phi = (D * D) * (1 / 2 - D * D * (5 + 3 * T1 + 10 * C1 - 4 * C1 * C1 - 9 * e0sq) / 24);
        phi = phi + Math.pow(D, 6) * (61 + 90 * T1 + 298 * C1 + 45 * T1 * T1 - 252 * e0sq - 3 * C1 * C1) / 720;
        phi = phi1 - (N1 * Math.tan(phi1) / R1) * phi;
        var lat = Math.floor(1000000 * phi / self.drad) / 1000000;
        ...
        var digraph = self.makeDigraph(x, y, utmz);
    ...
    def makeDigraph(x, y, utmz):
        ...
        var digraph = self.digraphLettersE.charAt(letter);
